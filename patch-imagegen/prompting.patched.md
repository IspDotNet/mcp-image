...
These prompting principles are primarily about how to request good images and good edits. When this skill is active, the validated default execution path is the global `mcp-image` MCP server.
...
- When the user asks for a specific visual medium, treat that as a hard constraint.

Examples of medium constraints:
...
- If `quality: "quality"` improves finish but reduces obedience, step back to `balanced`.

## Execution-layer defaults
- Use `mcp-image` as the default execution path.
- Assume `SKIP_PROMPT_ENHANCEMENT=true` is already configured in the validated Roo MCP setup.
- Default request preset: `quality: "balanced"`.
- Override `quality` only when the user explicitly wants faster drafts or highest fidelity.
- Keep `useWorldKnowledge` and `useGoogleSearch` off unless they materially improve correctness.
- Treat `purpose` as a minor execution-side bias, not as a substitute for a strong prompt.
