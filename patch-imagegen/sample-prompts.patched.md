...
These prompt recipes are starting points for requesting good images and edits. When this skill is active, the validated default execution path is `mcp-image`.

Default execution assumptions unless the user overrides them:
- `quality: "balanced"`
...
### historical-scene
For factual historical requests, consider enabling `useWorldKnowledge: true` at execution time.
...
```
Use case: illustration-story
Primary request: a cheerful dog jumping over a wooden farm gate in a countryside field
Style/medium: strictly 2D hand-drawn cartoon frame; authentic early-1960s animation style
Composition/framing: side view; full profile; clear silhouette
Constraints: flat cel-painted colors; simple hand-inked outlines; minimal shading; no 3D rendering; no volumetric lighting; no text; no watermark
```

### highest-quality-stylized-scene
For premium final renders where extra cost is acceptable, consider using the highest quality preset at execution time.

```
Use case: concept art
Primary request: a dramatic fantasy castle at dusk above a lake
Style/medium: painterly matte painting
Composition/framing: cinematic wide shot
Lighting/mood: moody twilight with reflections in the water
